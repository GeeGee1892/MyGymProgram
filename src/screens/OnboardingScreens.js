...
const calcCalories = (data) => {
  const bmr = Math.round(
    (10 * parseFloat(data.weight)) +
    (6.25 * parseFloat(data.height)) -
    (5 * parseFloat(data.age)) +
    (data.sex === 'female' ? -161 : 5)
  );
  const tdee = Math.round(bmr * 1.55); // Moderate activity
  const adjustment = { cut: -500, bulk: 300, maintain: 0 }[data.goal] || 0;
        ...
        <View style={styles.toggleRow}>
          {['male', 'female'].map((sex) => (
            <TouchableOpacity
              ...
              style={[
                styles.toggleButton,
                userData.gender === sex && styles.toggleButtonActive,
              ]}
            ...
            >
              <Text
                style={[
                  styles.toggleText,
                  userData.gender === sex && styles.toggleTextActive,
                ]}
              >
            ...
            <View style={styles.goalContent}>
              ...
              <Text
                style={[
                  ...
                  userData.goal === goal.id && styles.goalDescriptionActive,
                ]}
              >
                {goal.description}
              </Text>
        ...
        <View style={styles.daysRow}>
          {[3, 4, 5, 6].map((days) => (
            <TouchableOpacity
              ...
              style={[
                styles.dayButton,
                userData.trainingDaysPerWeek === days && styles.dayButtonActive,
              ]}
            ...
            >
              <Text
                style={[
                  styles.dayText,
                  userData.trainingDaysPerWeek === days && styles.dayTextActive,
                ]}
              >
                {days}
              </Text>
...
export const PlanReadyScreen = ({ navigation }) => {
  ...
  const handleStart = () => {
    ...
    completeOnboarding();
    navigation.reset({ index: 0, routes: [{ name: 'DailyCheckIn' }] });
  };
