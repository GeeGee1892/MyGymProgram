...
export const AnalyticsScreen = () => {
  ...
  const rangeCheckIns = checkIns
    .filter(c => {
      ...
      return !isNaN(checkInTime) && Date.now() - checkInTime < getRangeMs();
    })
    .sort((a, b) => new Date(a.date) - new Date(b.date));
    
  const rangeWorkouts = workouts.filter(w => {
    const workoutTime = new Date(w.date).getTime();
    return !isNaN(workoutTime) && Date.now() - workoutTime < getRangeMs();
  });

  // Weight calculations with NaN guards
  const weights = rangeCheckIns
    .map(c => parseFloat(c.weight))
    .filter(w => !isNaN(w) && w > 0);
    
  const currentWeight = weights[weights.length - 1] || parseFloat(userData?.weight) || 0;
  const startWeight = weights[0] || parseFloat(userData?.weight) || 0;
  ...
  const proteinRate = rangeCheckIns.length 
    ? Math.round((proteinHits / rangeCheckIns.length) * 100) 
    : 0;

  const workoutCount = rangeWorkouts.length;
  const workoutsByType = rangeWorkouts.reduce((acc, w) => {
    // Normalize type case
    const type = w.type ? w.type.charAt(0).toUpperCase() + w.type.slice(1).toLowerCase() : 'Other';
    acc[type] = (acc[type] || 0) + 1;
    return acc;
  }, {});

  // FIXED: Goal-aware color logic for weight change
  const getWeightChangeColor = () => {
    if (Math.abs(weightChange) < 0.1) return null; // No significant change
    
    const goal = userData?.goal || 'cut';
    
    if (goal === 'cut') {
      // Cutting: negative is good (green), positive is bad (red)
      return weightChange < 0 ? styles.positive : styles.negative;
    } else if (goal === 'bulk') {
      // Bulking: positive is good (green), negative is bad (red)
      return weightChange > 0 ? styles.positive : styles.negative;
    } else {
      // Maintain: any significant change is neutral (no color)
      return null;
    }
  };

  // FIXED: Get proper exercise name from exerciseDB
  const getExerciseName = (exerciseId) => {
    if (!exerciseId) return 'Unknown';
    const exercise = exerciseDB[exerciseId];
  ...
  const renderWeightChart = () => {
    // FIXED: Require at least 3 data points for meaningful trend
    if (weights.length < 3) {
      return (
        <View style={styles.emptyChart}>
          <Text style={styles.emptyText}>
            Need {3 - weights.length} more check-ins to show trend
          </Text>
        </View>
      );
    }
        ...
        <View style={styles.chartArea}>
            ...
            {weights.slice(-10).map((w, i, arr) => (
              <View 
                ...
                style={[
                  styles.chartBar, 
                  { 
                    height: ((w - min) / range) * chartHeight, 
                    backgroundColor: i === arr.length - 1 ? '#10b981' : '#3f3f46' 
                  }
                ]} 
              />
            ))}
          ...
          {userData?.targetWeight && (
            <View 
              style={[
                styles.targetLine, 
                { bottom: ((parseFloat(userData.targetWeight) - min) / range) * chartHeight }
              ]}
            >
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.statsRow}>
            ...
            <View style={styles.statCard}>
              {/* FIXED: Goal-aware color */}
              <Text style={[styles.statValue, getWeightChangeColor()]}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.adherenceCard}>
            ...
            <View style={styles.progressBar}>
              <View style={[styles.progressFill, { width: `${proteinRate}%` }]} />
            </View>
            <Text style={styles.adherenceSubtext}>
              Hit target {proteinHits} of {rangeCheckIns.length} days
            </Text>
          </View>
          <View style={styles.targetCard}>
            <View style={styles.targetRow}>
              <Text style={styles.targetText}>Daily Calories</Text>
              <Text style={styles.targetNumber}>{userData?.calories || 2200}</Text>
            </View>
            <View style={styles.targetRow}>
              <Text style={styles.targetText}>Daily Protein</Text>
              <Text style={styles.targetNumber}>{userData?.protein || 180}g</Text>
            </View>
          </View>
        </View>
            ...
            <View style={styles.workoutBreakdown}>
              {['Push', 'Pull', 'Legs'].map((type) => (
                <View key={type} style={styles.workoutType}>
                  <Text style={styles.workoutTypeCount}>
                    {workoutsByType[type] || 0}
                  </Text>
            ...
            <View style={styles.prList}>
              {Object.entries(allPRs).slice(0, 5).map(([exerciseId, pr]) => (
                <View key={exerciseId} style={styles.prCard}>
                  {/* FIXED: Proper exercise name lookup */}
                  <Text style={styles.prExercise}>{getExerciseName(exerciseId)}</Text>
                  <Text style={styles.prWeight}>
                    {typeof pr === 'object' ? pr.weight : pr}kg
                  </Text>
                </View>
              ))}
            </View>
...
const styles = StyleSheet.create({
  ...
  prExercise: { fontSize: 14, fontWeight: '500', color: '#fff' },
